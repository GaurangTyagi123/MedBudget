                ...
                <div className="min-w-full h-fit grid grid-cols-3 place-items-center mt-5 ">
                    {
                        columns.map((column: string,index:number) => {
                            return <span className="font-bold text-center" key={index}>{column}</span>
                        })
                    }
                </div>
                <form
                                ...
                                >
                                    <div className="flex flex-col items-center justify-between gap-y-2 w-md h-20">
                                        <input
                                            autoComplete="off"
                                            type="text"
                                            value={medicine[!index ? 'medicineName' : `medicineName-${index}`]}
                                            disabled={true}
                                    ...
                                    <div className="flex flex-col items-center justify-between gap-y-2 w-md h-20">
                                        <input
                                            ...
                                            defaultValue={formatCurrency(Number(medicine[!index ? 'medicinePrice' : `medicinePrice-${index}`]))}
                                            className="form_input"
                                            disabled={true}
                                    ...
                                    <div className="flex flex-col items-center justify-between gap-y-2 w-md h-20">
                                        <input
                                            autoComplete="off"
                                            type="text"
                                            id={medicine[!index ? 'medicineName' : `medicineName-${index}`] as string}
                                            defaultValue={1}
                                            className="form_input"
                                            required
                                            {...register(
                                                medicine[!index ? 'medicineName' : `medicineName-${index}`].toString(),
                                                {
                                                    valueAsNumber: true,
                                                    required:
                                        ...
                                        /> 
                                        {errors?.[medicine[!index ? 'medicineName' : `medicineName-${index}`]]
                                            ?.message && (
                                            <span className="text-red-500 font-bold text-xs uppercase">
                                                {String(
                                                    errors?.[
