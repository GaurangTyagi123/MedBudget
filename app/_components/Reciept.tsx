    ...
    return (
        <>
            <div className="relative space-y-5 md:h-[60%] md:w-[70%]  w-full h-full rounded-lg bg-stone-100/80 outline outline-mb-secondary-500 shadow-3xl shadow-mb-secondary-500 print:static print:translate-0 print:w-screen print:h-screen overflow-auto">
                <button className="h-12 w-12 text-2xl position absolute right-0  cursor-pointer" onClick={() => setShow?.(false)}>x</button>
                ...
                <div
                    className="px-5 flex flex-col items-center justify-center w-full gap-y-3 "
                >
                    {data.map(
                        (medicine, index) => {
                            return (
                    ...
                    <span className="hidden print:block">Total Cost : {formatCurrency(totalCost)}</span>
                </div>
            </div>
        </>
    );
